...
def printHelp():
	...
	######### Prints help section with all the commands and their usage:  #########
	print("\n\n")
...
linea = input(" > ")
parsed = CommandParser.parsear(linea)
comando = parsed[0]

######### Listening for user commands and executing them:
while comando != "exit":
	...
	######### The user wants us to show all the options and we do so:	
		...
		if len(parsed) > 1 and parsed[1] != "options":
			...
			if parsed[1] == "locip": print("\n  Local IP:",ServerClient.get_my_ip(),"\n")
	...
	######### All options ready, user wants to fire up the connection:
		...
		success = True
		if upnp:
			success = ServerClient.forwardPort(port,port,None,ip,False,"TCP",0,"Pylfer",True)
		...
		if mode == "server" and ipmode == "IPv4" and success:
			...
			if files:
				...
				if checkFileList(files):
					######### All ok, lets go:
					fileshare.ipv4FileServerFire(ip, port, whitebool, whitelist, blackbool, blacklist, files)
				else: print("  ERROR: There was a problem with at least one file on de file list... \n\n")
			else:
				fileshare.ipv4ChatServerFire(ip, port, whitebool, whitelist, blackbool, blacklist)
		######### Client connection is easier, not much to check:
		elif mode == "client" and ipmode == "IPv4" and success:
			fileshare.ipv4ClientFire(ip, port, files)
