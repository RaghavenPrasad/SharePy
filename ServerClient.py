...
def open_port(service_url,external_port,internal_client,internal_port=None,protocol='TCP',duration=0,description=None,enabled=1):
    ...
    # NewLeaseDuration can be any integer BUT some UPnP devices don't support it, so set it to 0 for better compatibility.
...
def forwardPort(eport, iport, router, lanip, disable, protocol, time, description, verbose):
    ...
    ######### This discover function is the one that fails when we are on a windows machine, we return false to handle the error on the main script:
    res = discover()
    if not res:
        return False
